...
class Functions_facets {
    ...
    public function getFacet($type, $value) {
        $results = $this->dbDriver->query('SELECT uid,pid,type,value FROM ' . $this->dbDriver->schemaName . '.facets WHERE type=\'' . pg_escape_string($type) . '\' AND normalize(value)=normalize(\'' . pg_escape_string($value) . '\') LIMIT 1');
    ...
    public function removeFacet($hash, $collectionName) {
        if ($this->facetExists($hash, $collectionName)) {
            $this->dbDriver->query('UPDATE ' . $this->dbDriver->schemaName . '.facets SET counter = counter - 1 WHERE uid=\'' . pg_escape_string($hash) . '\' AND collection=\'' . pg_escape_string($collectionName) . '\'', 500, 'Cannot delete facet for ' . $collectionName);
            return true;
        }
        return false;
    }
    ...
    private function getFacetsPivots($collectionName, $fields, $parentHash) {
         ...
         */
        $query = 'SELECT * FROM ' . $this->dbDriver->schemaName . '.facets WHERE counter > 0 AND ';
    ...
    private function facetExists($hash, $collectionName) {
        $query = 'SELECT 1 FROM ' . $this->dbDriver->schemaName . '.facets WHERE uid=\'' . pg_escape_string($hash) . '\' AND collection = \'' . pg_escape_string($collectionName) . '\'';
