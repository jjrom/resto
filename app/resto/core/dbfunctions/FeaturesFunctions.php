    ...
    public function storeFeature($id, $collection, $featureArray)
    {
        $keysValues = $this->featureArrayToKeysValues(
            ...
            array(
                ...
                'owner' => null,
                'status' => isset($featureArray['properties']) && isset($featureArray['properties']['status']) && ctype_digit($featureArray['properties']['status']) ? (integer) $featureArray['properties']['status'] : 1,
                'likes' => 0,
                'comments' => 0,
                'metadata' => array(),
                'published' => 'now()',
    ...
    public function updateFeature($feature, $collection, $newFeatureArray)
    {
        ...
        $keysAndValues = $this->featureArrayToKeysValues(
            ...
            array(
                ...
                'status' => isset($newFeatureArray['properties']) && isset($newFeatureArray['properties']['status']) && ctype_digit($newFeatureArray['properties']['status']) ? (integer) $newFeatureArray['properties']['status'] : $oldFeatureArray['properties']['status'],
