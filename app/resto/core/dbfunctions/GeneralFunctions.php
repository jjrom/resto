...
class GeneralFunctions
{
    ...
    {
        $keywords = array();
        $results = $this->dbDriver->query('SELECT name, normalize(name) as normalized, type, value, location FROM resto.keyword WHERE ' . 'lang IN(\'' . pg_escape_string($language) . '\', \'**\')' . (count($types) > 0 ? ' AND type IN(' . join(',', $types) . ')' : ''));
    ...
    public function getSharedLinkInitiator($resourceUrl, $token)
    {
        ...
        $results = $this->dbDriver->fetch($this->dbDriver->pQuery('SELECT userid FROM resto.sharedlink WHERE url=$1 AND token=$2 AND validity > now()', array($resourceUrl, $token)));
    ...
    public function createSharedLink($userid, $resourceUrl, $duration = 86400)
    {
        ...
        $results = $this->dbDriver->fetch($this->dbDriver->query('INSERT INTO resto.sharedlink (url, token, userid, validity) VALUES (\'' . pg_escape_string($resourceUrl) . '\',\'' . (RestoUtil::encrypt(mt_rand(0, 100000) . microtime())) . '\',' . pg_escape_string($userid) . ',now() + ' . $duration . ' * \'1 second\'::interval) RETURNING token', 500, 'Cannot share link'));
        if (count($results) === 1) {
            return array(
    ...
    {
        return $this->dbDriver->pQuery('INSERT INTO resto.log (userid,method,path,query,ip,querytime) VALUES ($1,$2,$3,$4,$5,now())', array(
            $userid ?? null,
            $query['method'] ?? null,
            $query['path'] ?? null,
            $query['query'] ?? null,
    ...
    {
        return !empty($this->dbDriver->fetch($this->dbDriver->pQuery('SELECT 1 FROM resto.revokedtoken WHERE token=$1', array($token))));
    }
    ...
    public function revokeToken($token, $validuntil)
    {
        if (isset($token) && !$this->isTokenRevoked($token)) {
            $this->dbDriver->pQuery('INSERT INTO resto.revokedtoken (token, validuntil) VALUES($1, $2)', array(
                $token,
                $validuntil ?? null
            ));
        }
     ...
     * @param array $params
     */
    private function getSplitterFunction($params) {
