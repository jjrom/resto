...
class GroupsFunctions
{
    ...
    public function getGroups($params = array())
    {
        ...
        if (!isset($params['viewAll']) || ! $params['viewAll']) {
            $where[] = 'id NOT IN (' . join(',', array(Resto::GROUP_ADMIN_ID, Resto::GROUP_DEFAULT_ID)) . ')';
        }
        ...
        return $this->formatGroups($this->dbDriver->fetch($this->dbDriver->query('SELECT id, name, description, owner, to_iso80601(created) as created FROM resto.group' . (count($where) > 0 ? ' WHERE ' . join(' AND ', $where) : '') . ' ORDER BY id DESC')), $params['id'] ?? null);
    }
    ...
    public function createGroup($params)
    {
        ...
        try {
            $result = pg_query_params($this->dbDriver->getConnection(), 'INSERT INTO resto.group (name, description, owner, created) VALUES ($1, $2, $3, now()) ON CONFLICT (normalize(name)) DO NOTHING RETURNING id ', array(
                $params['body']['name'],
    ...
    public function removeGroup($params)
    {
        ...
        try {
            if (isset($params['owner'])) {
                $result = pg_query_params($this->dbDriver->getConnection(), 'DELETE FROM resto.group WHERE id=($1) AND owner=($2)', array(
                    $params['id'],
                    $params['owner']
                ));
            } else {
                $result = pg_query_params($this->dbDriver->getConnection(), 'DELETE FROM resto.group WHERE id=($1)', array(
                    $params['id']
                ));
            }
    ...
    private function formatGroups($results, $groupId)
    {
        ...
        for ($i = $length; $i--;) {
            ...
            $results[$i]['created'] = $results[$i]['created'];
            if (! isset($results[$i]['owner'])) {
                unset($results[$i]['owner']);
            }
        }

        return isset($groupId) ? $results[0] : array(
                'groups' => $results
        );
    }
}
