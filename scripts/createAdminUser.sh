...
INSERT INTO ${DATABASE_SCHEMA}.user (id,email,groups,firstname,password,activated,registrationdate) VALUES (${ADMIN_USER_ID}, '${ADMIN_USER_NAME}','{0,100}','${ADMIN_USER_NAME}','${HASH}', 1, now_utc()) ON CONFLICT (id) DO UPDATE SET password='${HASH}';
EOF
else
PGPASSWORD=${DATABASE_USER_PASSWORD} psql -d ${DATABASE_NAME} -U ${DATABASE_USER_NAME} -h ${DATABASE_HOST_SEEN_FROM_DOCKERHOST} -p ${DATABASE_EXPOSED_PORT} > /dev/null 2> errors.log << EOF
INSERT INTO ${DATABASE_SCHEMA}.user (email,groups,firstname,password,activated,registrationdate) VALUES ('${ADMIN_USER_NAME}','{0,100}','${ADMIN_USER_NAME}','${HASH}', 1, now_utc()) ON CONFLICT (email) DO UPDATE SET password='${HASH}';
EOF
fi
